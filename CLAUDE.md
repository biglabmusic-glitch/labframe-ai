...
- `payment-webhook` — уведомление Продамуса об оплате: проверяет подпись, начисляет `credits`
...
## Оплата: Продамус + вебхук

**Почему не внутри мини-аппа.** Telegram запрещает продавать цифровые товары внутри
своих приложений за что-либо кроме Stars, и запрещает ссылки на внешние платёжки явно
(`core.telegram.org/bots/payments-stars`) — это требование App Store и Google Play.
Поэтому кнопка «Купить» в аппе ведёт в чат с владельцем, а ссылку на оплату владелец
выдаёт из админки. Stars как основной канал не годятся самозанятому: ~30% комиссии,
21 день заморозки, вывод только через TON и биржу, а чек в «Мой налог» нужен в рублях.

**Схема.** Админ дёргает `admin { action: 'payment-link', userId, packageId }` →
получает ссылку на форму Продамуса → шлёт покупателю → тот платит → Продамус зовёт
`payment-webhook` → баланс начислен, реферальная награда выдана.

**Что защищает вебхук.** Подпись HMAC-SHA256 в заголовке `Sign`
(`_shared/prodamus.ts`, алгоритм сверен с независимой реализацией, тест с эталонным
вектором в `prodamus_test.ts`). Плюс две проверки, которые подписью не покрываются:

- **Сумма сверяется с серверным прайсом.** Цена лежит в ссылке параметром, покупатель
  может занизить её в адресной строке до оплаты — Продамус честно подпишет уведомление
  на изменённую сумму. Расхождение → отказ в начислении (проверено на проде).
- **Идемпотентность по `order_id`.** Продамус повторяет уведомление, пока не получит 200.
  `apply_payment` вставляет платёж и начисляет баланс одной транзакцией, повтор
  возвращает false и баланс не трогает.

Адресат берётся из `order_id` формата `lf-<tgId>-<pkg>-<nonce>` — он входит в
подписанные данные, подделать нельзя.

**Осталось для запуска продаж** (регистрация на владельце — нужны паспорт и ИНН):

```powershell
supabase secrets set PRODAMUS_SECRET=<секретный ключ из настроек Продамуса>
supabase secrets set PRODAMUS_FORM_URL=https://<магазин>.payform.ru
supabase functions deploy payment-webhook --no-verify-jwt
supabase functions deploy admin --no-verify-jwt
```

Затем в кабинете Продамуса указать URL уведомлений:
`https://mmegdmfmozgaycuyeacl.supabase.co/functions/v1/payment-webhook`

Пока `PRODAMUS_SECRET` не задан, вебхук отвечает `500 not_configured` — это
сознательно: 500 заставит Продамус повторить уведомление, когда секрет появится,
а 200 молча потерял бы оплату.

После первой успешной оплаты можно удалить админ-экшен `mark-paid` — он был затычкой
до вебхука.
...
3. 🟡 **Бэкапы: автоматических нет, есть ручной скрипт.** У проекта бесплатный тариф,
   API отдаёт `"backups": []` и `pitr_enabled: false` — автобэкапов не будет.
   `supabase db dump` не годится: он гоняет pg_dump в Docker, а на машине владельца
   нет ни Docker, ни pg_dump.
   ...
   сознательно: она воспроизводится из `supabase/migrations/`, данные — ничем.
   Восстановление: миграции на пустую базу, затем этот файл в SQL Editor.

   Первый бэкап снят 24.08.2026 и проверен: все таблицы разбираются
   `json_populate_recordset`, число строк совпадает с базой.

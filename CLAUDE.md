...
0. 🟡 **Ветка `feat/credit-packages` не влита и не задеплоена.** Код готов, сборка
   и 11 тестов зелёные, но **миграция `0016` не применена ни к одной базе** — она сносит
   три триггера и переписывает балансы всем, поэтому сначала прогон на ветке проекта
   в Supabase и глазами проверить перенос:
   ```sql
   select id, username, is_admin, plan, usage_limit, usage_used, credits
     from public.users order by credits desc limit 20;
   ```
   Ждём: у админов 9999, у `plan = 'pro'` — 50, у остальных остаток не меньше 5.
   Затем деплой четырёх функций: `create-job`, `me`, `admin` (изменены сами) и
   `apply-referral` (тянет переписанный `_shared/referral.ts`).
   На Vercel добавить `VITE_OWNER_TG=DanyaSanta`.
1. Включить списание: `supabase secrets set LIMITS_DISABLED=0`. До этого баланс
   показывается, но не гейтит — удобно для проверки. Второго флага больше нет,
   `VITE_LIMITS_DISABLED` и `feature-flags.ts` удалены.
2. **Бэкапов базы нет вообще** (Supabase: «LAST BACKUP: No backups»). Теперь в базе
   лежат балансы, за которые люди платят деньги — закрыть раньше остального.
3. Ротировать секреты, которые юзер шарил в чате.
4. Платёжный вебхук на сайте вне Telegram: он начисляет `credits` и зовёт
...
5. Реферальный код теряется на `t.me/<bot>?start=ref_CODE` — бот payload не читает,
   работает только `?startapp=` на мини-апп. Чинить до запуска рекламы.
6. 152-ФЗ: в `users` лежат `first_name`, `last_name`, `photo_url` — это персональные

...
//
// Актуальный API Продамуса возвращает его в order_num, а в order_id кладёт
// собственный UUID — на этом мы уже обожглись, оплаты падали в «НЕ РАЗОБРАН».
// order_id оставлен вторым кандидатом ради классической формы, которая клала
// наше значение именно туда. Берём первое поле, которое разбирается в формат
// lf-<tgId>-<пакет>-<nonce>; чужой UUID под него всё равно не подойдёт.
const ORDER_FIELDS = ['order_num', 'order_id'];
...
Deno.serve(async (req) => {
  ...
  // Ключ идемпотентности — UUID Продамуса, а НЕ наш order_num, и это важно.
  // Наш номер живёт в ссылке: если человек откроет одну и ту же ссылку дважды
  // и дважды заплатит, order_num у обоих платежей совпадёт, и второй пакет
  // не начислился бы — деньги взяли бы, товар нет. UUID же у каждого платежа
  // свой, а на повторное уведомление о ТОМ ЖЕ платеже он не меняется, так что
  // от двойного начисления защищает по-прежнему.

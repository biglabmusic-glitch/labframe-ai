...
- В history лежат предыдущие готовые работы юзера (стиль/формат/тип/брендинг + дата).
- Прийми текущий выбор юзера (style/format/branding) как авторитет — не подменяй.
- НО внутри выбранного стиля подстраивайся под его «почерк»:
  - Если он чаще снимает crown → промт может тоньше учитывать форму единичной коронки.
  - Если повторяется один и тот же workType → подбери deeper-keywords (для виниров — translucency layers, для мостов — connector aesthetics).
  - Если history пустая — это первый пост юзера, выдай чистый шаблонный промт.
- В notes (по-русски) кратко скажи, что учёл из истории, либо «история пустая, базовый шаблон».
  ...
  const lines = [
    ...
    input.history.length
      ? `history (last ${input.history.length}, новейшая первая): ${input.history
          .map((h) => `${h.style}/${h.format}${h.workType ? `/${h.workType}` : ''}${h.branding ? `/${h.branding}` : ''}`)
          .join(', ')}`
      : 'history: empty',
  ].filter(Boolean);
  return lines.join('\n');
